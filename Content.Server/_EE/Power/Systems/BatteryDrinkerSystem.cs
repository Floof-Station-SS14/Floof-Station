...
public sealed partial class BatteryDrinkerSystem : EntitySystem
{
    ...
    private void OnDoAfter(EntityUid uid, BatteryDrinkerComponent drinkerComp, DoAfterEvent args)
    {
        ...
        _powerCell.TryGetBatteryFromSlot(drinker, out var attery);
        if (!_silicon.TryGetSiliconBattery(drinker, out var drinkerBattery))
            return;

        if (!TryComp<BatteryDrinkerSourceComponent>(source, out var sourceComp))
            return;
        if (!TryComp<BatteryComponent>(source, out var batteryComponent))
            return;


        var amountToDrink = MathF.Min(drinkerComp.DrinkMultiplier * 1000, _battery.GetCharge(source));
        amountToDrink = MathF.Min(amountToDrink, batteryComponent!.MaxCharge - _battery.GetCharge(drinkerBattery.Value.AsNullable()));
        ...
        if (_battery.TryUseCharge(source, amountToDrink))
            _battery.SetCharge(drinkerBattery.Value.AsNullable(), _battery.GetCharge(drinkerBattery.Value.AsNullable()) + amountToDrink);
        else
        {

            _battery.SetCharge(drinkerBattery.Value.AsNullable(), _battery.GetCharge(source) + _battery.GetCharge(drinker));
